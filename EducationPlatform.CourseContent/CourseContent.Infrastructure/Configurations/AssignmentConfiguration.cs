...
namespace CourseContent.Infrastructure.Configurations
{
    internal class AssignmentConfiguration : IEntityTypeConfiguration<Assignment>
    {
        public void Configure(EntityTypeBuilder<Assignment> builder)
        {
            ...
            builder.HasOne(d => d.Course).WithMany(p => p.Assignments).HasForeignKey(d => d.CourseId).HasConstraintName("fk_assignments_course");
            builder.HasOne(d => d.Topic).WithMany(t => t.Assignments).HasForeignKey(d => d.TopicId).HasConstraintName("fk_assignments_topics");
        }
    }
}
