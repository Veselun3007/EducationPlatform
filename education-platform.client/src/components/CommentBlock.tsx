import { Box, IconButton, Paper, Stack, TextField, Typography, useTheme } from '@mui/material';
...
const CommentBlock: React.FC<CommentBlockProps> = observer(({ comments, currentUser, sendComment, assignmentId }) => {
    ...
    return (
        <Box width="100%">
            <Stack width="100%" spacing={1} >
                <Stack width="100%" spacing={1} p={1} bgcolor={theme.palette.background.default}>
                    {comments.length > 0? comments.map((comment) => {
                        if (comment.comment.courseUserId === currentUser) {
                            return (
                    <Paper key={comment.comment.commentId} sx={{ width: 'fit-content', maxWidth: '50%', alignSelf: 'end', bgcolor: theme.palette.secondary.main, p: 1 }}>
                        <Stack height="100%" alignContent="center">
                            <Typography>{comment.comment.commentText}</Typography>

                            <Typography textAlign="left" variant="caption" color="InactiveCaptionText">
                                {comment.comment.commentDate.toLocaleString(i18n.language)}
                            </Typography>
                        </Stack>
                    </Paper>
                    );
                    ...
                    return (
                    <Stack key={comment.comment.commentId} direction="row" spacing={2} alignItems="center">
                        <ColoredAvatar
                        ...
                        <Paper sx={{ width: 'fit-content', maxWidth: '50%', alignSelf: 'start', p: 1 }}>
                            <Stack height="100%">
                                <Typography color={theme.palette.secondary.light}>{comment.userInfo.userName}</Typography>
                                <Typography>{comment.comment.commentText}</Typography>
                                <Typography textAlign="left" variant="caption" color="InactiveCaptionText">
                                    {comment.comment.commentDate.toLocaleString(i18n.language)}
                                </Typography>
                ...
                :<Typography textAlign="center" color="InactiveCaptionText">{t('glossary.noComments')} </Typography>
                }
                ...
                <Stack direction="row">
                    <TextField
                        ...
                        value={
                            store.createComment.commentText
                        }
