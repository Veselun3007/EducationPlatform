...
const MarkCard: React.FC<MarkCardProps> = observer(
    ({ saInfo, onSubmitMark, onFileClick, currentUser, sendComment, assignment }) => {
        const { t } = useTranslation();
        const navigate = useNavigate();
        const theme = useTheme();
        const store = useLocalObservable(
            () => ({
                updateMark: new UpdateMarkModel(
                ...
                get isValid() {
                    return (
                        this.updateMark.validateNewMark(
                            assignment.maxMark,
                            assignment.minMark,
                        ).length === 0
                    );
                ...
                reset() {
                    this.updateMark.newMark = saInfo.studentAssignment.currentMark
                        ? saInfo.studentAssignment.currentMark
                        : assignment.minMark;
                    this.error = null;
                },
                ...
                onMarkChange(e: React.ChangeEvent<HTMLInputElement>) {
                    if (e.target.value) {
                        ...
                        debounce(
                            action(() => {
                                const errors = this.updateMark.validateNewMark(
                                    assignment.maxMark,
                                    assignment.minMark,
                                );
            ...
            <Paper sx={{ width: '100%', p: 1 }}>
                <Stack width="100%" spacing={2}>
                    <Stack direction="row" alignItems="center" spacing={1}>
                        ...
                        />
                        <Typography flexGrow={1}>{saInfo.userInfo.userName}</Typography>
                        ...
                        {saInfo?.studentAssignment.isDone &&
                            ...
                            saInfo.files.length > 0 && (
                                <Typography>{t('glossary.submitted')}</Typography>
                            )}
                        ...
                        {saInfo?.studentAssignment.isDone &&
                            ...
                            saInfo.studentAssignment.submissionDate <
                                assignment!.assignmentDeadline &&
                            saInfo.files.length === 0 && (
                                <Typography>{t('glossary.markedAsDone')}</Typography>
                            )}
                    </Stack>
                    <Typography variant="h6">{t('glossary.attachedFiles')}</Typography>
                    ...
                    {saInfo.files.length > 0 ? (
                        <Grid2 container spacing={1}>
                            {saInfo.files.map((file) => (
                                ...
                                >
                                    <FileCard
                                        file={file.attachedFileName!.slice(
                                            file.attachedFileName!.indexOf('_') + 1,
                                        )}
                                        onClick={() =>
                                            onFileClick(file.attachedFileId, navigate)
                                        }
                                    />
                                </Grid2>
                            ))}
                    ...
                    <TextField
                        ...
                        required
                        label={t('common.mark')}
                        value={store.updateMark.newMark}
                    ...
                    <Button
                        variant="contained"
                        color="success"
                        onClick={() => onSubmitMark(store.updateMark, navigate)}
                    >
                        {saInfo.studentAssignment.currentMark
                            ? t('glossary.changeGrade')
                            : t('glossary.gradeWork')}
                    </Button>
                    <CommentBlock
                        comments={saInfo.comments}
                        currentUser={currentUser}
                        assignmentId={saInfo.studentAssignment.studentassignmentId}
