...
export default class ChatPageStore {
        ...
        try {
            ...
            this._hubConnection.on('ReceiveMessage', (response) => {
                if (Object.hasOwn(response, 'statusCode')) {
                    ...
                    runInAction(() => {
                        this.messages = this.messages
                            .concat(response as MessageModel)
                            .sort((a, b) => b.id - a.id);
                    });
                }
            });
                ...
                if (Object.hasOwn(response, 'statusCode')) {
                    ...
                    runInAction(() => {
                        ...
                        const message = this.messages.find(
                            (message) =>
                                message.attachedFiles?.some((file) => file.id === fileId),
                        );
                        if (message) {
    ...
    get isCreateMessageValid(): boolean {
        return (
            this.createMessage!.validateMessage() &&
    ...
    get isEditMessageValid(): boolean {
        ...
        return (
            this.editMessage!.validateMessageText().length === 0 &&
            ((message!.attachedFiles!.length === 0 &&
                this.editMessage!.messageText &&
                this.editMessage!.messageText!.length > 0) ||
                message!.attachedFiles!.length > 0)
        );
    }
    ...
    get canDeleteFile(): boolean {
        ...
        return (
            message !== undefined &&
            ((message.messageText !== null && message.messageText.length > 0) ||
                (message.attachedFiles !== null && message.attachedFiles.length > 1))
        );
    }
    ...
    onCreateMessageTextChange(e: React.ChangeEvent<HTMLInputElement>): void {
        ...
        debounce(
            action(() => {
                ...
                this.createMessageErrors.messageText =
    ...
    onEditMessageTextChange(e: React.ChangeEvent<HTMLInputElement>): void {
        ...
        debounce(
            action(() => {
                ...
                this.editMessageErrors.messageText =
                    errors.length !== 0 ? errors[0] : null;
            }),
        )();
    }
    ...
    handeEditMessageOpen() {
        ...
        this.editMessage = new UpdateMessageModel(
    ...
    async onEditMessageFileAdd(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            await this._hubConnection!.send(
                'AddMessageMedia',
                this.courseId,
                media,
                this.selectedMessage,
    ...
    async sendMessage() {
        try {
            ...
            const message = new CreateMessageModelTransport(
                this.createMessage!.courseId,
                this.createMessage!.creatorId,
                mediaMessages,
